...
test.describe('Navigation - Mobile', () => {
  ...
  test('should display hamburger button on mobile', async ({ page }) => {
    const burgerMenu = page.locator('.burger-menu, .hamburger, button[aria-label*="menu" i]');
  ...
  test('should open mobile menu on click', async ({ page }) => {
    ...
    if (await burgerMenu.count() > 0) {
      ...
      const mobileMenu = page.locator('#mobile-menu.active, .mobile-menu.active');
  ...
  test('should close mobile menu after navigation', async ({ page }) => {
    ...
    if (await burgerMenu.count() > 0) {
      ...
      if (await navLink.count() > 0) {
        ...
        const activeMobileMenu = page.locator('#mobile-menu.active, .mobile-menu.active');
        const count = await activeMobileMenu.count();
        // Either no active class exists, or the menu is hidden
        expect(count === 0 || !(await activeMobileMenu.first().isVisible().catch(() => false))).toBeTruthy();
      }
    }
  });
  ...
  test('should have correct aria-expanded attribute', async ({ page }) => {
    ...
    if (await burgerMenu.count() > 0) {
      const hasAriaExpanded = await burgerMenu.getAttribute('aria-expanded') !== null;
  ...
  test('should not have dead links in navigation', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (href && !href.startsWith('#') && !href.startsWith('http')) {
        ...
        const fullUrl = href.startsWith('/') ? href : `/frontend/${href}`;
