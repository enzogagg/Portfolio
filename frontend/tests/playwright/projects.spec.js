...
test.describe("Projects Page - Filtering Functionality", () => {
  ...
  // =========================================
  // TESTS for individual project pages
  // =========================================
  const projectPages = [
  ...
  for (const pageName of projectPages) {
    test.describe(`${pageName} - Project Page`, () => {
      test(`should load and display main content`, async ({ page }) => {
        ...
        // Use body instead of main as some project pages don't have <main> tag
        const container = page.locator("main, body");
        await expect(container.first()).toBeVisible();

        // Verify the page has loaded correctly - check for header/title
        const title = page.locator("h1, h2, .project-title, title");
  ...
  test("should filter projects by category", async ({ page }) => {
    ...
    if ((await filterButtons.count()) > 1) {
      ...
      const visibleCards = await projectCards
        .filter({ hasNotText: "" })
        .count();
  ...
  test("cards should display technologies used", async ({ page }) => {
    ...
    if ((await projectCards.count()) > 0) {
      ...
      const totalTags = await page
        .locator('.tech, .tag, [class*="technolog"]')
        .count();
